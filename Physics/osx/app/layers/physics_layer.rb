class PhysicsLayer < Joybox::Core::Layer
  ...
  def init_physics
    ...
    body = @world.new_body position:[0,0] do
      edge_fixture start_point: [0, 0], end_point: [Screen.width, 0]

      edge_fixture start_point: [0, Screen.height], end_point: [Screen.width, Screen.height]

      edge_fixture start_point: [0, Screen.height], end_point: [0, 0]

      edge_fixture start_point: [Screen.width, Screen.height], end_point: [Screen.width, 0]
    end
  end
  ...
  def layout_menu
    ...
    menu = Menu.new items: [reset_menu_item], position: Screen.center
  ...
  def add_new_sprite(position)
    ...
    body = @world.new_body position: position, type: Body::Dynamic do
